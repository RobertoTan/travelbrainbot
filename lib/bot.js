...
class TravelbrainBot {
  ...
  handleTextMessage(userId, message) {
    ...
    if (_.includes(['thank you', 'thanks', 'cheers'], text)) {
      this.sendTextMessage(userId, 'You are welcome. :)');
      return true;
    }
    ...
    if (q) {
      ...
      if (message === 'BOOK_HOTEL') {
        Promise.resolve()
          ...
          .then(() => {
            this.typingOff();
            this.sendTextMessage(userId, `No worries. I just booked it for you. Enjoy your trip!`);
          });
      }
    }
    return true;
  ...
  startReturningCustomerConversation(userId, payload) {
    this.lastItineraries[userId] = payload;

    this._getUserName(userId)
      .then(firstName => {
        let introMessage = `Hi ${firstName}. Have not seen you in a while. ;). Your travel agent just booked you into this flight. The flight is based on your preferences.`;
        let msg = utils.buildItineraryMessage(firstName, payload, introMessage);
        var messageData = {
          recipient: {
            id: userId
          },
      ...
      .then(() => {
        this.sendTextMessage(userId, 'And this booking has just been confirmed for you. Have a safe trip!');
        this.sendTextMessage(userId, '✈️');
      });
  }
